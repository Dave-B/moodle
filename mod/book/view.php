...
if ($book->navstyle) {
    ...
    if ($previd) {
        if ($book->navstyle == 1) {
            $chnavigation .= '<a title="' . get_string('navprev', 'book') . '" href="view.php?id=' . $cm->id . '&amp;chapterid=' . $previd .  '">' .
                '<img src="' . $OUTPUT->pix_url($navprevicon, 'mod_book') . '" class="icon" alt="' . get_string('navprev', 'book') . '"/></a>';
        } else {
            $chnavigation .= '<a title="' . get_string('navprev', 'book') . '" href="view.php?id=' . $cm->id . '&amp;chapterid=' . $previd . '">' .
                '<span class="chaptername"><span class="arrow">' . $OUTPUT->larrow() . '&nbsp;</span></span>' .
                get_string('navprev', 'book') . ':&nbsp;<span class="chaptername">' . $prevtitle . '</span></a>';
        }
    } else {
        if ($book->navstyle == 1) {
    ...
    if ($nextid) {
        if ($book->navstyle == 1) {
            $chnavigation .= '<a title="' . get_string('navnext', 'book') . '" href="view.php?id=' . $cm->id . '&amp;chapterid='.$nextid.'">' .
                '<img src="' . $OUTPUT->pix_url($navnexticon, 'mod_book').'" class="icon" alt="' . get_string('navnext', 'book') . '" /></a>';
        } else {
            $chnavigation .= ' <a title="' . get_string('navnext', 'book') . '" href="view.php?id=' . $cm->id . '&amp;chapterid='.$nextid.'">' .
                get_string('navnext', 'book') . ':<span class="chaptername">&nbsp;' . $nexttitle.
        ...
        if ($book->navstyle == 1) {
            $chnavigation .= '<a title="'.get_string('navexit', 'book').'" href="'.$returnurl.'">' .
                '<img src="' . $OUTPUT->pix_url('nav_exit', 'mod_book') . '" class="icon" alt="' . get_string('navexit', 'book') . '" /></a>';
        } else {
            $chnavigation .= ' <a title="' . get_string('navexit', 'book') . '" href="'.$returnurl.'">' .
                '<span class="chaptername">' . get_string('navexit', 'book') . '&nbsp;' . $OUTPUT->uarrow() . '</span></a>';
        }
...
if ($book->navstyle) {
    // Upper navigation.
    echo '<div class="navtop ' . $navclasses[$book->navstyle] . '">' . $chnavigation . '</div>';
}
...
if ($book->navstyle) {
    // Lower navigation.
    echo '<div class="navbottom ' . $navclasses[$book->navstyle] . '">' . $chnavigation . '</div>';
}
